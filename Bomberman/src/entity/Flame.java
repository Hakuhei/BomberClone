...
public class Flame extends Entity {
	...
	public Flame(int x, int y, int range, Sprite sprite, Arena level) {
		this.x = x;
		this.y = y;
		this.sprite = sprite;
		counter = 0;
	...
	public void render(Screen screen) {
		...
		for(int i = 1; i <= range; i++) {
			if(r == -1) {
				if(arena.getTile((x >> 6) + i, y >> 6).solid()) r = i - 1;
				if(arena.getTile((x >> 6) + i, y >> 6).breakable()) rightBreakable = i;
			}
			if(l == -1) {
				if(arena.getTile((x >> 6) - i, y >> 6).solid()) l = i - 1;
				if(arena.getTile((x >> 6) - i, y >> 6).breakable()) leftBreakable = i;
			}
			if(d == -1) {
				if(arena.getTile(x >> 6, (y >> 6) + i).solid()) d = i - 1;
				if(arena.getTile(x >> 6, (y >> 6) + i).breakable()) downBreakable = i;
			}
			if(u == -1) {
				if(arena.getTile(x >> 6, (y >> 6) - i).solid()) u = i - 1;
				if(arena.getTile(x >> 6, (y >> 6) - i).breakable()) upBreakable = i;
			}
		...
		for(int i = 1; i <= range; i++) {
			...
			if(counter == 25) {
				remove();
				if(rightBreakable == i) arena.addBrokenRocks((x >> 6) + i, y >> 6);
				if(leftBreakable == i) arena.addBrokenRocks((x >> 6) - i, y >> 6);
				if(downBreakable == i) arena.addBrokenRocks(x >> 6, (y >> 6) + i);
				if(upBreakable == i) arena.addBrokenRocks(x >> 6, (y >> 6) - i);
			}
